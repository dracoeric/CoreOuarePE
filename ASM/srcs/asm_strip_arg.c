...
/*   Updated: 2019/02/13 17:53:43 by erli             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	asm_get_arg(t_asm_data *data, char *line, int *nb_arg, int i)
{
	...
	if ((line[data->col] == '\0' || line[data->col] == '#')
		...
		return (0);
	if ((line[data->col] == ',' && nb_args == i + 1) || (line[data->col] != '\0'
		&& line[data->col] != ',' && nb_args == i))
		return (asm_error_msg(data, SYNTAX_ERROR));
	while (line[data->col] == ' ' || line[data->col] == '\t')
		data->col++;
	return (1);
				...
				int *cols)
{
	...
	{
		cols[i] = data->col;
		strips[i] = line + data->col;
		if ((ret = asm_get_arg(data, line, &ng_arg, 0)) < 0)
			return (-1);
		if (ret == 0)
			i = data->max_arg;
		i++;
